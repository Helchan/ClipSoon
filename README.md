...
- 支持历史容量、保留天数、粘贴延迟、图片批量间隔、选择后自动粘贴与失焦隐藏等设置；图片批量间隔默认 150 ms，可在 100–1000 ms 内调整。所有修改立即保存；除正在发送的图片批次继续使用启动时快照外，其余修改立即生效。设置采用无系统标题栏外壳，底部提供关闭与重置偏好，也可通过 `Esc` 或点击窗口外关闭。设置窗口打开期间，主面板会被模糊淡化到文字不可辨认，只保留大轮廓和色块，并停止响应搜索、列表、滚轮和右键操作。
- “磨砂”为默认主题，并与浅色、深色两种显式主题一同覆盖设置下拉列表、列表右键菜单等弹出组件，文字、悬停和选中状态均保持清晰对比。
- 主面板在三种主题下都使用静态细线边框表达圆角外缘，不绘制流动霓虹、发光光带或主面板投影，也不再提供对应设置开关；非 Windows 主题不再保留为投影预留的透明外缘空白，Windows 仍保持非分层窗口和物理圆角 mask，但顶层材质不在 mask 边缘额外描边，可见边框只保留主卡片内侧的一根 1 px 线。
...
```bash
git tag -a v1.1.11 -m "ClipSoon 1.1.11"
