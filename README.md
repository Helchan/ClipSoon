...
- 支持 Finder / 资源管理器式 `Shift`、`Ctrl` / `Command` 多选；右键菜单提供带单色语义图标的“收藏”“取消收藏”“删除”“清空”“清空F”“设置”：收藏状态不改变“全部”列表的最近排序，“收藏”Tab 单独展示收藏项；“清空F”清空全部收藏历史。菜单以紧凑的图标—文字间距和等值左右留白定宽，“设置”使用齿轮语义图标，清空确认采用与当前主题一致的应用内对话框。
...
- 磨砂质感（随系统）为默认主题，并与浅色、深色两种显式主题一同覆盖设置下拉列表、列表右键菜单等弹出组件，文字、悬停和选中状态均保持清晰对比。
- 磨砂质感使用一个应用内材质壳：环境色、双层折射感边缘、顶部高光和随指针移动的柔光共同构成效果，不会读取或截取桌面内容；macOS/Windows 原生材质可用时，底下窗口只能以被淡化的模糊色影和大轮廓透出，不能辨读底下文字或图标细节，前景内容仍保持清晰可读；列表和详情保持为可读的功能层，避免层层玻璃卡片。
- macOS 13+ 会在可用时叠加系统 `NSVisualEffectView` 背景模糊；Windows 11 22H2+ 会叠加官方 DWM Desktop Acrylic。两者都只是增强，Windows 10、系统材质不可用或没有 macOS Tahoe 时仍保留完整的应用内磨砂质感材质效果。
...
磨砂质感主题不依赖 macOS Tahoe。macOS 13+ 的系统背景模糊与 Windows 11 22H2（内部版本 22621）+
的 Desktop Acrylic 都是可选增强；Windows 10 仍受支持，并使用应用内材质渲染。原生背景模糊是否由
系统合成器真正呈现，仍必须在对应平台真机目视验收，自动化测试只能验证调用边界和应用内材质绘制。
...
```bash
git tag -a v1.0.1 -m "ClipSoon 1.0.1"
...
ClipSoon 的核心功能不发起网络请求。文本和文件记录保存在本机 SQLite 数据库中，图片以 PNG 文件保存在应用数据目录。你可随时在设置中暂停记录、打开数据目录或清空未收藏历史。
