...
剪贴板宿主使用 Win32 原生格式读取，主界面不调用 Qt MIME 读取，因此剪贴板竞争不会阻塞窗口或热键。发送图片时会同时发布位图、标准 `image/png` 和 Windows 注册格式 `PNG`，覆盖画图与 Chromium/Electron 聊天窗口常用的读取格式；ClipSoon 自身写回带内部标记，监听宿主不会再次读取整张图片并与目标程序争抢剪贴板。两个宿主每 500 ms 发送心跳，卡死、退出或父进程消失时会被自动替换或清理；大图在关闭系统剪贴板后继续转换落盘，重启宿主时自动回收无主临时文件。具体聊天客户端仍属于 Windows 真机验收项。错误统一写入主数据目录下的 `clipsoon.log`。
