...
- 支持 Finder / 资源管理器式 `Shift`、`Ctrl` / `Command` 多选，以及右键删除或清空历史；清空确认采用与当前主题一致的应用内对话框。
...
- 浅色、深色、跟随系统和玻璃半透（随系统明暗）四种主题覆盖设置下拉列表、列表右键菜单等弹出组件，文字、悬停和选中状态均保持清晰对比。
- 玻璃半透使用一个应用内材质壳：环境色、双层折射感边缘、顶部高光和随指针移动的柔光共同构成效果，不会读取或截取桌面内容；macOS/Windows 原生材质可用时，底下窗口只能以被淡化的模糊色影和大轮廓透出，不能辨读底下文字或图标细节，前景内容仍保持清晰可读；列表和详情保持为可读的功能层，避免层层玻璃卡片。
- macOS 13+ 会在可用时叠加系统 `NSVisualEffectView` 背景模糊；Windows 11 22H2+ 会叠加官方 DWM Desktop Acrylic。两者都只是增强，Windows 10、系统材质不可用或没有 macOS Tahoe 时仍保留完整的应用内玻璃半透材质效果。
...
玻璃半透主题不依赖 macOS Tahoe。macOS 13+ 的系统背景模糊与 Windows 11 22H2（内部版本 22621）+
