...
- 支持 Finder / 资源管理器式 `Shift`、`Ctrl` / `Command` 多选；按 `Enter` 会按当前列表从上到下严格串行发送全部选中项，各项之间自动插入一个换行，完整发送成功时末尾不追加换行，原条目内容保持不变。换行通过剪贴板作为字面文本粘贴，不模拟可能触发聊天软件发送动作的 Enter；上一项和换行步骤都完成剪贴板写入、校验、粘贴触发及目标消费等待后，才处理下一项。任一真实项或换行步骤失败即停止剩余项并按真实项报告进度。多选发送需要开启“选择后自动粘贴”并从目标应用呼出面板，因为系统剪贴板本身不能同时保存多个独立的文本、图片与文件条目。右键菜单提供带单色语义图标和右侧快捷键提示的“全选”“收藏”“取消收藏”“删除”“清空”“清空NF”“设置”：收藏状态不改变“全部”列表的最近排序，“收藏”Tab 单独展示收藏项；收藏、取消收藏和删除不会让列表跳回顶部，删除后自动接续选中下一项或上一项；“清空NF”清空全部非收藏历史并保留收藏项。菜单以紧凑的图标—文字间距和等值左右留白定宽，“设置”使用齿轮语义图标，清空确认采用与当前主题一致的应用内对话框。
...
- `Enter` 发送全部选中项并在项间自动换行，双击发送点击项；`Esc` 或面板失去焦点后隐藏。Windows 首次从全局快捷键呼出时也能识别外部点击。
...
| `Enter` | 按列表顺序将全部选中项发送到原应用，项间自动换行 |
| `Esc` | 隐藏面板 |
| 点击放大镜 | 打开设置 |
