...
- `Enter` 发送全部选中项，双击发送点击项；`Esc` 或面板失去焦点后隐藏。Windows 首次从全局快捷键呼出时也能识别外部点击。
...
```bash
git tag -a v1.1.3 -m "ClipSoon 1.1.3"
