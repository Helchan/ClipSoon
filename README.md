...
- 磨砂使用一个固定浅色的应用内材质壳：环境色、双层折射感边缘、顶部高光和随指针移动的柔光共同构成效果，不会读取或截取桌面内容，不跟随系统明暗，也不启用平台原生背景模糊；底部和右侧信息区保持浅亮可读，列表和详情保持为可读的功能层，避免层层玻璃卡片。
...
```bash
git tag -a v1.0.2 -m "ClipSoon 1.0.2"
