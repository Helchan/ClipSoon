...
def test_settings_dialog_applies_changes_and_reset_without_a_save_step(qtbot, tmp_path) -> None:
    ...
    application._apply_settings(legacy_values, dialog)
    assert not hasattr(JsonSettingsStore(tmp_path / "settings.json").load(), "neon_border_enabled")
    assert not hasattr(application.panel, "neon_border")
